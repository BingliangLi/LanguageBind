...
def split_processing(input_folder, output_folder, num_gpus=8):
    ...
    # Get all audio files that need processing
...
if __name__ == '__main__':
    ...
    # Calculate number of processes based on CPU limitation (e.g., 60% utilization)
    num_cpus = os.cpu_count()
    num_processes = max(1, int(num_cpus * 0.4))  # Use at least one process

    # Process the audios in parallel
    split_processing(input_folder, output_folder, num_processes)
